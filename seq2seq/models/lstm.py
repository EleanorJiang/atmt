...
class LSTMEncoder(Seq2SeqEncoder):
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        # logging.debug(self.dictionary.pad_idx)
...
class AttentionLayer(nn.Module):
    """ Defines the attention layer class. Uses Luong's global attention with the general scoring function.
        See: https://machinelearningmastery.com/the-luong-attention-mechanism/
    """
...
class LSTMDecoder(Seq2SeqDecoder):
    ...
    def forward(self, tgt_inputs, encoder_out, incremental_state=None):
        ...
        logging.debug("tgt_time_steps: %s", tgt_time_steps)
        logging.debug("decoder_output: %s",decoder_output.size())
        logging.debug("attn_weights: %s", attn_weights.size())
